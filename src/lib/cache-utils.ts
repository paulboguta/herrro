...
export const cacheInvalidation = {
  ...
  account: {
    ...
    updateOne: (queryClient: QueryClient, accountId: string, updater: (old: any) => any) => {
  ...
  transaction: {
    ...
    addOptimistic: (queryClient: QueryClient, accountId: string, newTransaction: any) => {
      // Add to account-specific list
      queryClient.setQueriesData({ queryKey: transactionKeys.byAccount(accountId) }, (old: any) => {
        if (!old || !Array.isArray(old)) return [newTransaction];
        return [newTransaction, ...old];
      });

      // Add to general list
      queryClient.setQueriesData({ queryKey: transactionKeys.lists() }, (old: any) => {
        if (!old || !Array.isArray(old)) return [newTransaction];
        return [newTransaction, ...old];
      });
    },
...
export const prefetchHelpers = {
  ...
  prefetchAccount: async (queryClient: QueryClient, accountId: string, fetcher: () => Promise<any>) => {
  ...
  prefetchTransactions: async (queryClient: QueryClient, accountId: string, fetcher: () => Promise<any>) => {
    return queryClient.prefetchQuery({
      queryKey: transactionKeys.byAccount(accountId),
      queryFn: fetcher,
